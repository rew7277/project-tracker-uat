...
function Dashboard({cu,tasks,projects,users,onNav,activeTeam,teams,setTeamCtx}){
  ...
  const health=cc?cc.health_score:0, risk=cc?cc.risk_score:0;
  const filteredTeams=useMemo(()=>safe(teams).filter(x=>(x.name||'').toLowerCase().includes(teamSearch.toLowerCase())),[teams,teamSearch]);
  const stageData=cc?safe(cc.stage_distribution).map(x=>({name:(STAGES[x.name]&&STAGES[x.name].label)||x.name,value:x.value})):[];
  const priData=cc?safe(cc.priority_distribution).map(x=>({name:(PRIS[x.name]&&PRIS[x.name].label)||x.name,value:x.value})):[];
  const topProjects=cc?safe(cc.project_health):[];
  const workload=cc?safe(cc.workload):[];
  const insights=cc?safe(cc.insights):[];
  const cmdItems=[
  ...
  return html`<div class="ptx-shell">
    ...
    ${loading?html`<div class="pt-kpis"><${PTSkeleton}/><${PTSkeleton}/><${PTSkeleton}/><${PTSkeleton}/><${PTSkeleton}/></div><div class="pt-grid"><${PTSkeleton} h=${330}/><${PTSkeleton} h=${330}/></div>`:html`
      ...
      <div class="pt-grid"><div class="pt-card"><div class="pt-card-head"><h3>Throughput and capacity</h3><span class="pt-chip">14 days</span></div>${RC&&RC.ResponsiveContainer?html`<${RC.ResponsiveContainer} width="100%" height=${240}><${RC.LineChart} data=${cc.throughput||[]} margin=${{top:10,right:10,left:-20,bottom:0}}><${RC.CartesianGrid} strokeDasharray="3 3" opacity=${0.25}/><${RC.XAxis} dataKey="day" fontSize=${10}/><${RC.YAxis} fontSize=${10}/><${RC.Tooltip}/><${RC.Line} type="monotone" dataKey="created" stroke="#2563eb" strokeWidth=${2} dot=${false}/><${RC.Line} type="monotone" dataKey="completed" stroke="#16a34a" strokeWidth=${2} dot=${false}/><${RC.Line} type="monotone" dataKey="hours" stroke="#7c3aed" strokeWidth=${2} dot=${false}/></${RC.LineChart}></${RC.ResponsiveContainer}>`:html`<div class="pt-empty-mini">Charts unavailable.</div>`}</div>
