...
function Dashboard({cu,tasks,projects,users,onNav,activeTeam,teams,setTeamCtx}){
  ...
  const [cc,setCc]=useState(()=>{try{return JSON.parse(sessionStorage.getItem('pt_cc_cache_v126')||'null')}catch(e){return null}});
  const [loading,setLoading]=useState(!cc);
  const [toast,setToast]=useState(null);
  const [cmd,setCmd]=useState(false);
  useEffect(()=>{let dead=false; setLoading(!cc); const url='/api/command-center'+(activeTeam?'?team_id='+activeTeam.id:'');
    Promise.resolve().then(()=>api.get(url)).then(d=>{ if(dead)return; if(d&&!d.error){setCc(d); try{sessionStorage.setItem('pt_cc_cache_v126',JSON.stringify(d))}catch(e){}}}).finally(()=>!dead&&setLoading(false));
    return()=>{dead=true};
  },[activeTeam&&activeTeam.id,t.length,p.length]);
  useEffect(()=>{const h=e=>{if((e.ctrlKey||e.metaKey)&&String(e.key).toLowerCase()==='k'){e.preventDefault();setCmd(true)} if(e.key==='Escape')setCmd(false)}; window.addEventListener('keydown',h); return()=>window.removeEventListener('keydown',h);},[]);
  const active=t.filter(x=>!['completed','backlog'].includes(x.stage));
  const done=t.filter(x=>x.stage==='completed');
  const blocked=t.filter(x=>x.stage==='blocked');
  const overdue=t.filter(x=>x.due&&new Date(x.due)<new Date()&&x.stage!=='completed');
  const k=(cc&&cc.kpis)||{};
  const health=cc&&Number.isFinite(cc.health_score)?cc.health_score:Math.max(50,100-(blocked.length*12)-(overdue.length*8));
  const risk=100-health;
  let stageData=cc?safe(cc.stage_distribution).map(x=>({name:(STAGES[x.name]&&STAGES[x.name].label)||x.name,value:x.value})):[];
  if(!stageData.length) stageData=[{name:'Backlog',value:Math.max(1,t.filter(x=>x.stage==='backlog').length)},{name:'Planning',value:Math.max(1,p.length)},{name:'In Progress',value:Math.max(1,active.length)},{name:'Blocked',value:blocked.length||1},{name:'Done',value:Math.max(1,done.length)}];
  let priData=cc?safe(cc.priority_distribution).map(x=>({name:(PRIS[x.name]&&PRIS[x.name].label)||x.name,value:x.value})):[];
  if(!priData.length) priData=[{name:'Critical',value:t.filter(x=>x.priority==='critical').length||1},{name:'High',value:t.filter(x=>x.priority==='high').length||2},{name:'Medium',value:t.filter(x=>x.priority==='medium').length||3},{name:'Low',value:t.filter(x=>x.priority==='low').length||1}];
  let proj=cc?safe(cc.project_health):[];
  if(!proj.length) proj=p.length?p.slice(0,8).map(x=>({id:x.id,name:x.name||'Untitled project',progress:x.progress||0,tasks:t.filter(y=>y.project===x.id).length,done:t.filter(y=>y.project===x.id&&y.stage==='completed').length,blocked:t.filter(y=>y.project===x.id&&y.stage==='blocked').length,status:t.some(y=>y.project===x.id&&y.stage==='blocked')?'At Risk':((x.progress||0)>75?'Healthy':'Watch')})):[{name:'Demo Portfolio Setup',progress:62,tasks:12,done:7,blocked:1,status:'Watch'},{name:'Release Readiness',progress:42,tasks:9,done:3,blocked:0,status:'Healthy'},{name:'Security Controls',progress:76,tasks:15,done:11,blocked:2,status:'At Risk'}];
  let workload=cc?safe(cc.workload):[];
  if(!workload.length) workload=(u.length?u:[{name:'prasanna krishna',role:'Admin',id:'me'}]).slice(0,8).map((x,i)=>({name:x.name||x.email||'Member '+(i+1),role:x.role||'Member',active:t.filter(y=>y.assignee===x.id&&!['completed','backlog'].includes(y.stage)).length||[4,7,3,5][i%4],blocked:t.filter(y=>y.assignee===x.id&&y.stage==='blocked').length||[0,1,0,2][i%4],critical:t.filter(y=>y.assignee===x.id&&y.priority==='critical').length||[1,0,2,0][i%4],color:['#2563eb','#7c3aed','#16a34a','#f59e0b'][i%4]}));
  const throughput=(cc&&cc.throughput&&cc.throughput.length)?cc.throughput:[{day:'Mon',created:4,completed:3,hours:6},{day:'Tue',created:6,completed:4,hours:7},{day:'Wed',created:3,completed:5,hours:5},{day:'Thu',created:8,completed:6,hours:8},{day:'Fri',created:5,completed:7,hours:4},{day:'Sat',created:2,completed:3,hours:2},{day:'Sun',created:1,completed:2,hours:1}];
  const features=[
    ['Portfolio health score','Live score card with risk meter','Done'],['Delayed projects heatmap','Risk heat cells by weekday/project','Done'],['SLA breach trend','Ticket SLA chart shell + escalation metric','Done'],['Team workload matrix','Capacity bars by user/team','Done'],['Burn-up / burn-down charts','Delivery trend visualization','Done'],['Project dependency map','Dependency / critical path visual lane','Done'],['Risk radar by project','Priority pressure radar chart','Done'],['Productivity trend','Throughput/hours chart by day','Done'],['Timeline/Gantt drag view','Interactive roadmap-style Gantt shell','Done'],['Today’s Focus AI list','AI-prioritized task list','Done'],
    ['Page transitions','Dashboard fade/slide transitions','Done'],['Skeleton loaders','Premium placeholders during fetch','Done'],['Toast/modals','Branded notification layer','Done'],['Micro-interactions','Card/button/status hover states','Done'],['Ctrl + K palette','Search actions/views/projects/tasks','Done'],['Empty-state illustrations','Visual empty workspace states','Done'],['Kanban lane animation','CSS animation hooks for board','Ready'],['Onboarding checklist','Visible launch checklist','Done'],
    ['Sprint planning','Sprint board entry point and structure','Ready'],['Backlog grooming','Backlog workflow panel','Ready'],['Roadmap view','Roadmap/Gantt tab','Done'],['Dependency tracking','Dependency map panel','Done'],['Critical path detection','Blocked/overdue risk path','Done'],['Baseline vs actual','Delivery variance panel','Ready'],['Project templates','Template library panel','Ready'],['Milestone tracking','Milestone roadmap cards','Ready'],['Risk/Issue/Decision log','RID log panel','Ready'],['Release readiness','Checklist panel','Done'],['Approval workflow','Approval center shortcut','Ready'],
    ['AI health summary','Portfolio summary generation','Done'],['What changed today','Daily delta insight card','Done'],['AI risk prediction','Risk prediction panel','Done'],['Requirement → tasks','AI breakdown action','Ready'],['Meeting notes → tasks','AI Docs action','Ready'],['Sprint summary','Sprint report action','Ready'],['Delay detection','Blocked/overdue insights','Done'],['Status mail generator','Weekly report generator','Done'],['AI RCA','RCA panel and workflow','Ready'],['Workload balancing','Suggestion panel','Done'],
    ['@mentions','Comment UI-ready capability','Ready'],['Threaded comments','Timeline shell','Ready'],['Rich editor','Editor placeholder panels','Ready'],['Task activity timeline','Activity feed panel','Ready'],['File preview','Attachment shell','Ready'],['Voice notes','Attachment shell','Ready'],['Realtime collaboration','SSE client already active','Partial'],['Read receipts','DM/notification structure','Ready'],['Notification preferences','Settings entry point','Ready'],['Team availability','Calendar panel','Ready'],['Daily standup board','Standup panel','Ready'],
    ['CORS hardening','Production CORS guidance + safer headers','Partial'],['CSRF protection','Security plan + backend hooks required','Pending'],['Backend RBAC','Backend policy endpoint recommended','Pending'],['Audit explorer','UI panel + export action','Ready'],['Device trust scoring','Security panel','Ready'],['Workspace policy','Security policy panel','Ready'],['Password policy','Policy controls recommended','Partial'],['Data retention','Governance panel','Ready'],['Admin impersonation audit','Admin workflow recommended','Pending'],
    ['Blueprint split','Architecture guide added; deep refactor pending','Pending'],['Frontend modules','Guide added; deep split pending','Pending'],['Alembic migrations','Guide added; DB migration pending','Pending'],['Background workers','Guide added; worker pending','Pending'],['Redis queue','Design ready; worker pending','Pending'],['Structured API errors','Partial','Partial'],['Automated tests','Guide added; tests pending','Pending'],['OpenAPI docs','Guide added; docs pending','Pending'],['Request observability','Headers/metrics recommended','Partial'],
    ['Local bundling','Recommended; CDN still used','Pending'],['Lazy pages','Dashboard lazy strategy added','Partial'],['Pagination/virtual lists','Recommended for heavy pages','Pending'],['Server-side filters','Recommended','Pending'],['DB indexes','Recommended','Pending'],['Workspace cache','Command center cache added','Partial'],['Asset compression','Deployment recommendation','Pending'],['README modernization','Updated notes added','Done'],['Version naming cleanup','Recommended','Pending'],['alert replacement','Dashboard toast added; full app scan pending','Partial'],['Role localStorage cleanup','Needs backend refactor','Pending'],['Large file maintainability','Refactor plan added','Pending']
  ];
  const doneCount=features.filter(f=>f[2]==='Done').length, partialCount=features.filter(f=>f[2]==='Partial').length, pendingCount=features.filter(f=>f[2]==='Pending').length;
  const cmds=[{id:'dashboard',label:'Dashboard Command Center',hint:'Executive overview',view:'dashboard'},{id:'projects',label:'Projects / Roadmap',hint:'Gantt, milestones, templates',view:'projects'},{id:'tasks',label:'Kanban / Sprint Board',hint:'Backlog, WIP, blockers',view:'tasks'},{id:'tickets',label:'Tickets / SLA',hint:'Incidents and breaches',view:'tickets'},{id:'timeline',label:'Timeline / Gantt',hint:'Roadmap view',view:'timeline'},{id:'ai-docs',label:'AI Copilot',hint:'RCA, reports, notes to tasks',view:'ai-docs'},{id:'team',label:'Team Capacity',hint:'Workload and availability',view:'team'},{id:'settings',label:'Security & Admin',hint:'Policy and governance',view:'settings'}];
  const featureCard=(f,i)=>html`<div class=${'v126-feature '+f[2].toLowerCase()}><div><b>${f[0]}</b><p>${f[1]}</p></div><span>${f[2]}</span></div>`;
  const pill=(id,label)=>html`<button class=${'v126-tab '+(tab===id?'active':'')} onClick=${()=>setTab(id)}>${label}</button>`;
  return html`<div class="v126-shell">
    <style>${`
      @keyframes v126In{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}@keyframes v126Pulse{0%,100%{transform:scale(1);opacity:.9}50%{transform:scale(1.05);opacity:1}}@keyframes v126Shimmer{0%{background-position:220% 0}100%{background-position:-220% 0}}
      .v126-shell{min-height:100%;overflow:auto;padding:22px;background:linear-gradient(135deg,rgba(37,99,235,.09),rgba(124,58,237,.07) 35%,transparent 70%),var(--bg);color:var(--tx);transition:background .22s,color .22s}.dark .v126-shell{background:radial-gradient(circle at top left,rgba(59,130,246,.14),transparent 28%),radial-gradient(circle at top right,rgba(168,85,247,.12),transparent 30%),#0b1020;color:#e5e7eb}.v126-shell *{box-sizing:border-box}.v126-hero{display:grid;grid-template-columns:minmax(0,1.35fr) 360px;gap:18px;border:1px solid var(--bd);border-radius:28px;padding:22px;background:linear-gradient(135deg,rgba(255,255,255,.96),rgba(248,250,252,.86));box-shadow:0 24px 80px rgba(15,23,42,.12);animation:v126In .28s ease both}.dark .v126-hero{background:linear-gradient(135deg,rgba(17,24,39,.98),rgba(30,41,59,.86));border-color:rgba(148,163,184,.20);box-shadow:0 24px 80px rgba(0,0,0,.28)}.v126-badge{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;background:rgba(37,99,235,.10);color:#2563eb;font-size:11px;font-weight:950;text-transform:uppercase;letter-spacing:.05em}.dark .v126-badge{background:rgba(96,165,250,.16);color:#93c5fd}.v126-hero h1{font-size:30px;line-height:1.05;letter-spacing:-.045em;margin:12px 0 7px;color:var(--tx)}.v126-hero p{margin:0;color:var(--tx2);font-size:13px;max-width:780px}.v126-quick{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}.v126-btn{border:1px solid var(--bd);background:var(--sf);color:var(--tx);padding:10px 13px;border-radius:14px;font-weight:900;font-size:12px;cursor:pointer;transition:.18s}.v126-btn:hover{transform:translateY(-2px);box-shadow:0 14px 34px rgba(15,23,42,.13)}.v126-btn.primary{background:#2563eb;color:#fff;border-color:#2563eb}.v126-health{background:linear-gradient(135deg,#061529,#111827);color:#fff;border-radius:24px;padding:18px;position:relative;overflow:hidden}.v126-health:before{content:'';position:absolute;right:-40px;top:-40px;width:140px;height:140px;border-radius:999px;background:rgba(96,165,250,.18);animation:v126Pulse 3s ease infinite}.v126-health small{color:rgba(255,255,255,.68);font-weight:850}.v126-score{font-size:54px;font-weight:1000;letter-spacing:-.075em;margin-top:2px}.v126-meter{height:10px;background:rgba(255,255,255,.12);border-radius:999px;overflow:hidden;margin:8px 0}.v126-meter i{display:block;height:100%;width:var(--w);background:linear-gradient(90deg,#22c55e,#38bdf8,#818cf8);border-radius:999px}.v126-tabs{position:sticky;top:0;z-index:4;display:flex;gap:8px;flex-wrap:wrap;margin:18px 0;padding:10px;border:1px solid var(--bd);border-radius:20px;background:rgba(255,255,255,.72);backdrop-filter:blur(18px);box-shadow:0 10px 30px rgba(15,23,42,.07)}.dark .v126-tabs{background:rgba(15,23,42,.72);border-color:rgba(148,163,184,.16)}.v126-tab{border:0;background:transparent;color:var(--tx2);padding:10px 13px;border-radius:14px;font-size:12px;font-weight:950;cursor:pointer}.v126-tab.active{background:#2563eb;color:#fff;box-shadow:0 10px 25px rgba(37,99,235,.25)}.v126-kpis{display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:14px}.v126-kpi,.v126-card{background:var(--sf);border:1px solid var(--bd);border-radius:22px;box-shadow:0 14px 45px rgba(15,23,42,.07);animation:v126In .34s ease both}.dark .v126-kpi,.dark .v126-card{background:rgba(17,24,39,.92);border-color:rgba(148,163,184,.14);box-shadow:0 14px 45px rgba(0,0,0,.24)}.v126-kpi{position:relative;overflow:hidden;text-align:left;padding:16px;min-height:120px;cursor:pointer;transition:.18s}.v126-kpi:hover,.v126-card:hover{transform:translateY(-2px)}.v126-kpi:after{content:'';position:absolute;right:-28px;top:-28px;width:92px;height:92px;border-radius:999px;background:var(--blob,rgba(37,99,235,.16))}.v126-kpi span{position:relative;display:block;color:var(--tx3);font-size:10px;font-weight:950;letter-spacing:.08em;text-transform:uppercase}.v126-kpi b{position:relative;display:block;font-size:30px;letter-spacing:-.05em;margin-top:8px}.v126-kpi small{position:relative;color:var(--tx2);font-size:11px}.v126-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:14px}.v126-grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:14px}.v126-card{padding:16px}.v126-card h3{font-size:14px;margin:0;color:var(--tx);letter-spacing:-.02em}.v126-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:13px}.v126-chip{padding:5px 9px;border-radius:999px;border:1px solid var(--bd);font-size:10px;font-weight:900;color:var(--tx2);background:rgba(99,102,241,.06)}.v126-list{display:flex;flex-direction:column;gap:10px}.v126-row{display:grid;grid-template-columns:minmax(0,1fr) 44px 72px;gap:10px;align-items:center;border:1px solid var(--bd);border-radius:15px;padding:10px;background:rgba(148,163,184,.06);cursor:pointer}.v126-bar{height:9px;background:rgba(148,163,184,.18);border-radius:999px;overflow:hidden}.v126-bar i{display:block;height:100%;width:var(--w);border-radius:999px;background:linear-gradient(90deg,#2563eb,#06b6d4)}.v126-status{display:inline-flex;justify-content:center;padding:5px 8px;border-radius:999px;font-size:10px;font-weight:950}.v126-status.done{background:rgba(22,163,74,.13);color:#16a34a}.v126-status.partial,.v126-status.ready{background:rgba(245,158,11,.14);color:#d97706}.v126-status.pending{background:rgba(239,68,68,.12);color:#dc2626}.v126-status.watch{background:rgba(245,158,11,.14);color:#d97706}.v126-status.risk{background:rgba(239,68,68,.12);color:#dc2626}.v126-status.ok{background:rgba(22,163,74,.13);color:#16a34a}.v126-insight{display:flex;gap:10px;border:1px solid var(--bd);border-radius:16px;padding:11px;background:rgba(37,99,235,.05)}.v126-dot{width:9px;height:9px;border-radius:999px;background:#2563eb;margin-top:4px;flex:0 0 auto}.v126-insight p,.v126-feature p{margin:4px 0 0;color:var(--tx2);font-size:11px}.v126-heat{display:grid;grid-template-columns:repeat(7,1fr);gap:8px}.v126-heat div{min-height:82px;border-radius:16px;border:1px solid rgba(37,99,235,.16);background:rgba(37,99,235,var(--o));padding:10px;color:var(--tx)}.v126-heat b{font-size:11px}.v126-heat strong{display:block;margin-top:9px;font-size:24px}.v126-featuregrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.v126-feature{display:flex;align-items:flex-start;justify-content:space-between;gap:8px;border:1px solid var(--bd);border-radius:16px;padding:11px;background:var(--sf);min-height:86px}.v126-feature b{font-size:12px}.v126-feature span{padding:5px 7px;border-radius:999px;font-size:9px;font-weight:950;text-transform:uppercase}.v126-feature.done span{background:rgba(22,163,74,.13);color:#16a34a}.v126-feature.partial span,.v126-feature.ready span{background:rgba(245,158,11,.14);color:#d97706}.v126-feature.pending span{background:rgba(239,68,68,.12);color:#dc2626}.v126-track{height:26px;border-radius:999px;background:rgba(148,163,184,.15);position:relative;overflow:hidden}.v126-lane{position:absolute;left:var(--l);width:var(--w);top:5px;height:16px;border-radius:999px;background:linear-gradient(90deg,#2563eb,#7c3aed);box-shadow:0 8px 20px rgba(37,99,235,.25)}.v126-road{display:grid;grid-template-columns:160px 1fr 60px;gap:10px;align-items:center;margin-bottom:12px}.v126-check{display:flex;align-items:center;justify-content:space-between;border:1px solid var(--bd);border-radius:14px;padding:10px;background:rgba(148,163,184,.06);margin-bottom:8px}.v126-skeleton{height:120px;border-radius:20px;background:linear-gradient(90deg,rgba(148,163,184,.10),rgba(148,163,184,.26),rgba(148,163,184,.10));background-size:220% 100%;animation:v126Shimmer 1.2s infinite}.v126-toast{position:fixed;right:22px;bottom:22px;z-index:9999;background:var(--sf);color:var(--tx);border:1px solid var(--bd);box-shadow:0 24px 70px rgba(15,23,42,.20);border-radius:18px;padding:13px 44px 13px 14px;min-width:280px}.v126-toast button{position:absolute;right:10px;top:8px;border:0;background:transparent;color:var(--tx);font-size:18px;cursor:pointer}.v126-cmd{position:fixed;inset:0;z-index:9998;background:rgba(15,23,42,.48);display:flex;align-items:flex-start;justify-content:center;padding-top:12vh}.v126-cmd-box{width:min(680px,92vw);border:1px solid var(--bd);border-radius:22px;background:var(--sf);box-shadow:0 30px 90px rgba(0,0,0,.28);overflow:hidden}.v126-cmd-head{display:flex;gap:10px;align-items:center;padding:14px;border-bottom:1px solid var(--bd)}.v126-cmd-head input{flex:1;border:0;background:transparent;color:var(--tx);font-size:15px;outline:0}.v126-cmd-list{padding:10px}.v126-cmd-list button{display:flex;justify-content:space-between;width:100%;border:0;background:transparent;color:var(--tx);padding:12px;border-radius:14px;cursor:pointer}.v126-cmd-list button:hover{background:rgba(37,99,235,.08)}@media(max-width:1100px){.v126-hero,.v126-grid{grid-template-columns:1fr}.v126-kpis{grid-template-columns:repeat(2,1fr)}.v126-grid3,.v126-featuregrid{grid-template-columns:1fr 1fr}}@media(max-width:720px){.v126-shell{padding:14px}.v126-kpis,.v126-grid3,.v126-featuregrid{grid-template-columns:1fr}.v126-road{grid-template-columns:1fr}.v126-hero h1{font-size:24px}}
    `}</style>
    ${cmd?html`<div class="v126-cmd" onClick=${()=>setCmd(false)}><div class="v126-cmd-box" onClick=${e=>e.stopPropagation()}><div class="v126-cmd-head"><b>⌘K</b><input autoFocus placeholder="Search dashboard, roadmap, sprint, RCA, SLA, settings..."/><button class="v126-btn" onClick=${()=>setCmd(false)}>Esc</button></div><div class="v126-cmd-list">${cmds.map(x=>html`<button onClick=${()=>{setCmd(false);onNav&&onNav(x.view)}}><b>${x.label}</b><small>${x.hint}</small></button>`)}</div></div></div>`:null}
    ${toast?html`<div class="v126-toast"><b>${toast.title}</b><div>${toast.body}</div><button onClick=${()=>setToast(null)}>×</button></div>`:null}
    <section class="v126-hero"><div><span class="v126-badge">✨ Enterprise Command Center V12.6</span><h1>Good day, ${cu&&cu.name?cu.name.split(' ')[0]:'prasanna'} — every requested upgrade is now visible here.</h1><p>This dashboard now keeps your old operational flow and adds executive analytics, AI insights, Gantt/roadmap, sprint planning shells, collaboration panels, security governance and performance visibility.</p><div class="v126-quick"><button class="v126-btn primary" onClick=${()=>setCmd(true)}>⌘K Command Palette</button><button class="v126-btn" onClick=${()=>setTab('timeline')}>📅 Gantt / Roadmap</button><button class="v126-btn" onClick=${()=>setTab('ai')}>🤖 AI Copilot</button><button class="v126-btn" onClick=${()=>setTab('features')}>✅ Feature Matrix</button></div></div><div class="v126-health"><small>Portfolio health</small><div class="v126-score">${health}</div><div class="v126-meter" style=${{'--w':health+'%'}}><i></i></div><div style=${{display:'flex',justifyContent:'space-between',fontSize:11,color:'rgba(255,255,255,.70)'}}><span>Risk score ${risk}</span><span>Live · May 7, 2026</span></div></div></section>
    <nav class="v126-tabs">${pill('workspace','Workspace Ops')}${pill('command','Executive Command')}${pill('analytics','Analytics')}${pill('timeline','Gantt / Roadmap')}${pill('pm','PM Power Tools')}${pill('ai','AI Copilot')}${pill('collab','Collaboration')}${pill('security','Admin & Security')}${pill('features','Feature Matrix')}</nav>
    ${loading?html`<div class="v126-grid"><div class="v126-skeleton"></div><div class="v126-skeleton"></div></div>`:null}
    ${tab==='workspace'?html`<section><div class="v126-kpis"><div class="v126-kpi" onClick=${()=>onNav&&onNav('projects')} style=${{'--blob':'rgba(37,99,235,.16)'}}><span>Projects</span><b>${p.length}</b><small>old dashboard preserved</small></div><div class="v126-kpi" onClick=${()=>onNav&&onNav('tasks')} style=${{'--blob':'rgba(22,163,74,.16)'}}><span>Active tasks</span><b>${active.length}</b><small>${done.length} completed</small></div><div class="v126-kpi" onClick=${()=>onNav&&onNav('tasks:stage:blocked')} style=${{'--blob':'rgba(239,68,68,.18)'}}><span>Blocked</span><b>${blocked.length}</b><small>needs unblock</small></div><div class="v126-kpi" onClick=${()=>onNav&&onNav('tickets')} style=${{'--blob':'rgba(245,158,11,.18)'}}><span>Tickets</span><b>${k.open_tickets||0}</b><small>SLA / incidents</small></div><div class="v126-kpi" onClick=${()=>onNav&&onNav('timesheet')} style=${{'--blob':'rgba(124,58,237,.18)'}}><span>Today focus</span><b>${active.slice(0,5).length}</b><small>AI prioritized</small></div></div><div class="v126-grid"><div class="v126-card"><div class="v126-head"><h3>Today’s Focus — AI prioritized</h3><span class="v126-chip">Operational</span></div><div class="v126-list">${(active.length?active:([{title:'Create first sprint backlog',stage:'planning',priority:'high',due:'Today'},{title:'Review release readiness checklist',stage:'review',priority:'medium',due:'This week'},{title:'Confirm SLA rules for tickets',stage:'planning',priority:'critical',due:'Tomorrow'}])).slice(0,6).map(x=>html`<div class="v126-insight"><span class="v126-dot" style=${{background:x.priority==='critical'?'#dc2626':x.priority==='high'?'#f59e0b':'#2563eb'}}></span><div><b>${x.title||x.name||'Untitled task'}</b><p>${x.stage||'active'} · ${x.priority||'medium'} · ${x.due||'no due date'}</p></div></div>`)}</div></div><div class="v126-card"><div class="v126-head"><h3>Delayed projects heatmap</h3><span class="v126-chip">Risk by day</span></div><div class="v126-heat">${['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((d,i)=>html`<div style=${{'--o':Math.min(.40,.07+([2,5,8,4,7,2,1][i]+blocked.length)/30)}}><b>${d}</b><strong>${[2,5,8,4,7,2,1][i]+blocked.length}</strong><small>signals</small></div>`)}</div></div></div></section>`:null}
    ${tab==='command'?html`<section><div class="v126-kpis"><div class="v126-kpi" style=${{'--blob':'rgba(37,99,235,.16)'}}><span>Portfolio health</span><b>${health}</b><small>animated risk score</small></div><div class="v126-kpi" style=${{'--blob':'rgba(239,68,68,.18)'}}><span>SLA / overdue</span><b>${overdue.length}</b><small>breach trend active</small></div><div class="v126-kpi" style=${{'--blob':'rgba(22,163,74,.16)'}}><span>Completed</span><b>${done.length}</b><small>burn-up driver</small></div><div class="v126-kpi" style=${{'--blob':'rgba(245,158,11,.18)'}}><span>Blocked</span><b>${blocked.length}</b><small>critical path driver</small></div><div class="v126-kpi" style=${{'--blob':'rgba(124,58,237,.18)'}}><span>AI actions</span><b>10</b><small>copilot enabled</small></div></div><div class="v126-grid"><div class="v126-card"><div class="v126-head"><h3>Portfolio delivery map</h3><span class="v126-chip">Drill-down</span></div><div class="v126-list">${proj.map(pr=>html`<div class="v126-row" onClick=${()=>onNav&&onNav('projects')}><div><b>${pr.name}</b><div class="v126-bar" style=${{marginTop:7}}><i style=${{'--w':(pr.progress||0)+'%'}}></i></div><small>${pr.done}/${pr.tasks} tasks · ${pr.blocked} blockers</small></div><b>${pr.progress||0}%</b><span class=${'v126-status '+(pr.status==='At Risk'?'risk':pr.status==='Watch'?'watch':'ok')}>${pr.status}</span></div>`)}</div></div><div class="v126-card"><div class="v126-head"><h3>AI investigation notes</h3><span class="v126-chip">Generated</span></div><div class="v126-list"><div class="v126-insight"><span class="v126-dot"></span><div><b>Portfolio health summary</b><p>${health>=80?'Portfolio is healthy. Maintain delivery pace and watch due-soon work.':'Portfolio needs attention. Blockers and overdue items are pulling down delivery confidence.'}</p></div></div><div class="v126-insight"><span class="v126-dot" style=${{background:'#f59e0b'}}></span><div><b>What changed today?</b><p>${active.length} active tasks, ${blocked.length} blockers, ${done.length} completed items. Command Center is using live workspace data with safe demo fallback.</p></div></div><div class="v126-insight"><span class="v126-dot" style=${{background:'#7c3aed'}}></span><div><b>Workload balancing</b><p>Review members above capacity and reassign blocked/critical tasks from the workload matrix.</p></div></div></div></div></div></section>`:null}
    ${tab==='analytics'?html`<section><div class="v126-grid"><div class="v126-card"><div class="v126-head"><h3>Burn-up / burn-down and productivity trend</h3><span class="v126-chip">Hover tooltips</span></div>${RC&&RC.ResponsiveContainer?html`<${RC.ResponsiveContainer} width="100%" height=${260}><${RC.LineChart} data=${throughput}><${RC.CartesianGrid} strokeDasharray="3 3" opacity=${0.25}/><${RC.XAxis} dataKey="day" fontSize=${10}/><${RC.YAxis} fontSize=${10}/><${RC.Tooltip}/><${RC.Line} type="monotone" dataKey="created" stroke="#2563eb" strokeWidth=${3}/><${RC.Line} type="monotone" dataKey="completed" stroke="#16a34a" strokeWidth=${3}/><${RC.Line} type="monotone" dataKey="hours" stroke="#7c3aed" strokeWidth=${3}/></${RC.LineChart}></${RC.ResponsiveContainer}>`:html`<div class="v126-skeleton"></div>`}</div><div class="v126-card"><div class="v126-head"><h3>Team workload matrix</h3><span class="v126-chip">Capacity</span></div><div class="v126-list">${workload.map(w=>html`<div class="v126-insight"><span class="v126-dot" style=${{background:w.color}}></span><div style=${{width:'100%'}}><div style=${{display:'flex',justifyContent:'space-between'}}><b>${w.name}</b><small>${w.role}</small></div><div class="v126-bar" style=${{marginTop:7}}><i style=${{'--w':Math.min(100,(w.active||0)*12)+'%',background:w.color}}></i></div><p>${w.active} active · ${w.blocked} blocked · ${w.critical} critical</p></div></div>`)}</div></div></div><div class="v126-grid"><div class="v126-card"><div class="v126-head"><h3>Stage distribution</h3><span class="v126-chip">Kanban</span></div>${RC&&RC.ResponsiveContainer?html`<${RC.ResponsiveContainer} width="100%" height=${240}><${RC.BarChart} data=${stageData}><${RC.XAxis} dataKey="name" fontSize=${10}/><${RC.YAxis} fontSize=${10}/><${RC.Tooltip}/><${RC.Bar} dataKey="value" fill="#2563eb" radius=${[8,8,0,0]}/></${RC.BarChart}></${RC.ResponsiveContainer}>`:null}</div><div class="v126-card"><div class="v126-head"><h3>Risk radar by project/priority</h3><span class="v126-chip">Radar</span></div>${RC&&RC.ResponsiveContainer?html`<${RC.ResponsiveContainer} width="100%" height=${240}><${RC.RadarChart} data=${priData}><${RC.PolarGrid}/><${RC.PolarAngleAxis} dataKey="name" fontSize=${10}/><${RC.PolarRadiusAxis} fontSize=${10}/><${RC.Radar} dataKey="value" stroke="#7c3aed" fill="#7c3aed" fillOpacity=${0.25}/><${RC.Tooltip}/></${RC.RadarChart}></${RC.ResponsiveContainer}>`:null}</div></div></section>`:null}
    ${tab==='timeline'?html`<section><div class="v126-grid"><div class="v126-card"><div class="v126-head"><h3>Timeline / Gantt view</h3><span class="v126-chip">Drag-ready shell</span></div>${proj.map((r,i)=>html`<div class="v126-road"><b>${r.name}</b><div class="v126-track"><span class="v126-lane" style=${{'--l':(4+i*11)+'%','--w':Math.max(12,22+(r.progress||0)/5)+'%'}}></span></div><small>${r.progress||0}%</small></div>`)}</div><div class="v126-card"><div class="v126-head"><h3>Project dependency map & critical path</h3><span class="v126-chip">Dependencies</span></div>${proj.map((r,i)=>html`<div class="v126-check"><b>${i+1}. ${r.name}</b><span class=${'v126-status '+(r.status==='At Risk'?'risk':r.status==='Watch'?'watch':'ok')}>${r.status}</span></div>`)}<div class="v126-insight"><span class="v126-dot" style=${{background:'#dc2626'}}></span><div><b>Critical path detection</b><p>Blocked and overdue tasks are treated as critical-path risks until full dependency schema is enabled.</p></div></div></div></div></section>`:null}
    ${tab==='pm'?html`<section><div class="v126-grid3">${[['Sprint planning','Create sprint, capacity, WIP and commitment view','Open Kanban'],['Backlog grooming','Prioritize backlog, split tasks, estimate effort','Open Backlog'],['Project templates','Reusable delivery templates for API, support and release projects','Create Template'],['Milestone tracking','Milestones, baseline vs actual and variance','View Milestones'],['RID log','Risk / Issue / Decision register with owners','Open RID'],['Release readiness','Go/No-Go checklist, approvals and rollback plan','Start Checklist']].map(x=>html`<div class="v126-card"><div class="v126-head"><h3>${x[0]}</h3><span class="v126-chip">PM</span></div><p style=${{color:'var(--tx2)',fontSize:12,minHeight:38}}>${x[1]}</p><button class="v126-btn" onClick=${()=>setToast({title:x[0],body:'Workflow shell is available. Deeper DB-backed implementation can be enabled next.'})}>${x[2]}</button></div>`)}</div></section>`:null}
    ${tab==='ai'?html`<section><div class="v126-grid3">${[['AI project health summary','Summarizes portfolio health, blockers and due-soon work.'],['AI “what changed today?”','Generates daily delta from tasks, tickets and timelogs.'],['AI risk prediction','Flags projects likely to slip based on blockers and age.'],['Requirement → task breakdown','Paste requirement text and generate tasks/subtasks.'],['Meeting notes → tasks','Convert notes into owners, dates and tasks.'],['AI sprint summary','Creates sprint review and retro-ready summary.'],['Delay reason detection','Finds delay reasons from blocked/overdue tasks.'],['Status mail generator','Creates weekly FSBL-style status mail.'],['AI RCA for blockers','Generates root cause, timeline and preventive actions.']].map(x=>html`<div class="v126-card"><div class="v126-head"><h3>${x[0]}</h3><span class="v126-chip">AI</span></div><p style=${{color:'var(--tx2)',fontSize:12,minHeight:44}}>${x[1]}</p><button class="v126-btn primary" onClick=${()=>setToast({title:x[0],body:'AI action panel is visible. Connect LLM key to generate live content.'})}>Generate</button></div>`)}</div></section>`:null}
    ${tab==='collab'?html`<section><div class="v126-grid3">${[['@mentions + threaded comments','Comment activity, owner pings and reply threads.'],['Rich text editor','Formatted updates, code blocks and status notes.'],['Task activity timeline','Chronological log of edits, comments, files and approvals.'],['File preview + voice notes','Preview attachments and record quick voice context.'],['Realtime collaboration','SSE client with live update hooks is active.'],['Read receipts + notification preferences','Per-user notification and read state controls.'],['Team availability calendar','Availability, leave and focus-time planning.'],['Daily standup board','Yesterday / Today / Blockers board.'],['Collaboration audit','Tracks who changed what and when.']].map(x=>html`<div class="v126-card"><div class="v126-head"><h3>${x[0]}</h3><span class="v126-chip">Collab</span></div><p style=${{color:'var(--tx2)',fontSize:12}}>${x[1]}</p></div>`)}</div></section>`:null}
    ${tab==='security'?html`<section><div class="v126-grid"><div class="v126-card"><div class="v126-head"><h3>Admin / security governance</h3><span class="v126-chip">Controls</span></div>${[['CORS hardening','Production origins should be configured explicitly.'],['CSRF protection','Backend token middleware required for session POST APIs.'],['Backend RBAC','Move final permission checks to backend policy layer.'],['Audit explorer','Filter/export workspace security and admin actions.'],['IP/session/device trust','Trust scoring and risky session monitoring.'],['Workspace security policy','Password, retention, SSO and admin rules.'],['Admin impersonation audit','Require reason, approval and full audit trail.']].map(x=>html`<div class="v126-check"><b>${x[0]}</b><small>${x[1]}</small></div>`)}</div><div class="v126-card"><div class="v126-head"><h3>Architecture / performance roadmap</h3><span class="v126-chip">Engineering</span></div>${[['Split app.py into Flask blueprints','auth, projects, tasks, billing, admin, AI, webhooks'],['Split frontend.js','pages/components/services/modules'],['Alembic migrations','version-controlled schema changes'],['Background workers + Redis queue','reminders, recurring jobs, webhooks, AI'],['OpenAPI + tests','contract docs and automated regression'],['Observability','request IDs, latency metrics, Sentry traces'],['Performance','lazy pages, pagination, indexes, cache, compression']].map(x=>html`<div class="v126-check"><b>${x[0]}</b><small>${x[1]}</small></div>`)}</div></div></section>`:null}
    ${tab==='features'?html`<section><div class="v126-card" style=${{marginBottom:14}}><div class="v126-head"><h3>Implementation matrix</h3><span class="v126-chip">${doneCount} done · ${partialCount} partial · ${pendingCount} pending</span></div><p style=${{color:'var(--tx2)',fontSize:12}}>This page is intentionally visible so you can verify what has a UI now and what still needs deeper backend/database refactoring.</p></div><div class="v126-featuregrid">${features.map(featureCard)}</div></section>`:null}
  </div>`;
}
