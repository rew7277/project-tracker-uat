...
function Dashboard({cu,tasks,projects,users,onNav,activeTeam,teams,setTeamCtx}){
  ...
  const [cc,setCc]=useState(null),[loading,setLoading]=useState(true),[toast,setToast]=useState(null),[cmd,setCmd]=useState(false);
  const [teamDropOpen,setTeamDropOpen]=useState(false),[teamSearch,setTeamSearch]=useState('');
  const teamDropRef=useRef(null);
  useEffect(()=>{const h=e=>{if(teamDropRef.current&&!teamDropRef.current.contains(e.target))setTeamDropOpen(false)};document.addEventListener('mousedown',h);return()=>document.removeEventListener('mousedown',h);},[]);
  useEffect(()=>{setLoading(true);api.get('/api/command-center'+(activeTeam?'?team_id='+activeTeam.id:'')).then(d=>{setCc(d&&!d.error?d:null);}).finally(()=>setLoading(false));},[activeTeam&&activeTeam.id,t.length,p.length]);
  useEffect(()=>{const h=e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();setCmd(true)}if(e.key==='Escape')setCmd(false)};window.addEventListener('keydown',h);return()=>window.removeEventListener('keydown',h);},[]);
  const k=cc&&cc.kpis?cc.kpis:{};
  const health=cc?cc.health_score:86, risk=cc?cc.risk_score:14;
  const filteredTeams=useMemo(()=>safe(teams).filter(x=>(x.name||'').toLowerCase().includes(teamSearch.toLowerCase())),[teams,teamSearch]);
  let stageData=cc?safe(cc.stage_distribution).map(x=>({name:(STAGES[x.name]&&STAGES[x.name].label)||x.name,value:x.value})):[]; if(!stageData.length)stageData=[{name:'Planning',value:3},{name:'Development',value:5},{name:'Testing',value:2},{name:'Blocked',value:1},{name:'Completed',value:4}];
  let priData=cc?safe(cc.priority_distribution).map(x=>({name:(PRIS[x.name]&&PRIS[x.name].label)||x.name,value:x.value})):[]; if(!priData.length)priData=[{name:'Critical',value:1},{name:'High',value:3},{name:'Medium',value:6},{name:'Low',value:2}];
  let topProjects=cc?safe(cc.project_health):[]; if(!topProjects.length)topProjects=[{name:'Sample Digital Lending Rollout',progress:72,tasks:18,done:13,blocked:1,status:'Watch'},{name:'Mobile App Stabilisation',progress:91,tasks:24,done:22,blocked:0,status:'Healthy'},{name:'API Security Hardening',progress:48,tasks:14,done:6,blocked:3,status:'At Risk'}];
  let workload=cc?safe(cc.workload):[]; if(!workload.length)workload=[{name:'Prasanna',role:'Admin',active:4,blocked:1,critical:1,color:'#2563eb'},{name:'Developer Team',role:'Engineering',active:7,blocked:2,critical:1,color:'#7c3aed'},{name:'QA Team',role:'Testing',active:3,blocked:0,critical:0,color:'#16a34a'}];
  let insights=cc?safe(cc.insights):[]; if(!insights.length)insights=[{type:'good',title:'Command Center ready',detail:'Add projects, tasks and tickets to convert this demo surface into live portfolio analytics.'},{type:'deadline',title:'Smart visual mode enabled',detail:'Charts, workload, risk radar and AI notes will update automatically from workspace data.'}];
  const cmdItems=[
    {id:'dashboard',label:'Open Dashboard',hint:'Executive command center',view:'dashboard'},
    {id:'projects',label:'Open Projects',hint:'Portfolio, owners, progress',view:'projects'},
    {id:'tasks',label:'Open Kanban Board',hint:'Stages, blockers, priorities',view:'tasks'},
    {id:'timeline',label:'Open Timeline / Gantt',hint:'Delivery dates and time health',view:'timeline'},
    {id:'tickets',label:'Open Tickets',hint:'Open incidents and support work',view:'tickets'},
    {id:'timesheet',label:'Open Timesheet',hint:'Logged hours and capacity',view:'timesheet'},
    {id:'ai-docs',label:'Open AI Docs',hint:'Generate summaries and docs',view:'ai-docs'},
    {id:'blocked',label:'Show Blocked Work',hint:(k.blocked_tasks||0)+' blocker(s)',action:()=>onNav&&onNav('tasks:stage:blocked')},
  ];
  const mkSub=(n,txt)=>`${n||0} ${txt}`;
  return html`<div class="ptx-shell">
    <style>${`
      @keyframes ptxFade{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}@keyframes ptxPulse{0%,100%{opacity:.42;transform:scale(1)}50%{opacity:.85;transform:scale(1.08)}}@keyframes ptxShimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
      .ptx-shell{height:100%;overflow:auto;padding:18px 22px 26px;background:radial-gradient(circle at top left,rgba(37,99,235,.12),transparent 32%),radial-gradient(circle at 80% 0,rgba(124,58,237,.10),transparent 28%),var(--bg);color:var(--tx)}
      .pt-hero{position:relative;overflow:hidden;border:1px solid var(--bd);border-radius:24px;padding:22px;background:linear-gradient(135deg,rgba(255,255,255,.92),rgba(248,250,252,.78));box-shadow:0 20px 70px rgba(15,23,42,.10);animation:ptxFade .38s ease both}.pt-hero:before{content:"";position:absolute;inset:-80px auto auto -80px;width:260px;height:260px;border-radius:999px;background:rgba(37,99,235,.16);filter:blur(8px);animation:ptxPulse 5s ease-in-out infinite}.pt-hero-grid{position:relative;display:grid;grid-template-columns:1.3fr .7fr;gap:18px;align-items:center}.pt-eyebrow{display:inline-flex;gap:8px;align-items:center;padding:5px 10px;border-radius:999px;background:rgba(37,99,235,.09);color:#1d4ed8;font-size:11px;font-weight:800;letter-spacing:.04em;text-transform:uppercase}.pt-hero h1{margin:10px 0 6px;font-size:28px;line-height:1.05;letter-spacing:-.04em}.pt-hero p{margin:0;color:var(--tx2);font-size:13px;max-width:720px}.pt-actions{display:flex;gap:9px;flex-wrap:wrap;margin-top:16px}.pt-action{border:1px solid var(--bd);background:var(--sf);color:var(--tx);padding:9px 12px;border-radius:13px;font-weight:800;font-size:12px;cursor:pointer;transition:.18s}.pt-action:hover{transform:translateY(-2px);box-shadow:0 10px 28px rgba(15,23,42,.12)}
      .pt-health{border-radius:22px;padding:17px;background:linear-gradient(135deg,#07111f,#111827);color:white;box-shadow:inset 0 1px 0 rgba(255,255,255,.12)}.pt-score{font-size:48px;font-weight:900;letter-spacing:-.06em}.pt-ring{height:10px;background:rgba(255,255,255,.12);border-radius:999px;overflow:hidden}.pt-ring>i{display:block;height:100%;border-radius:999px;background:linear-gradient(90deg,#22c55e,#60a5fa);width:var(--w);transition:width .8s ease}.pt-risk{display:flex;justify-content:space-between;margin-top:9px;font-size:11px;color:rgba(255,255,255,.72)}
      .pt-kpis{display:grid;grid-template-columns:repeat(5,minmax(140px,1fr));gap:12px;margin-top:14px}.pt-kpi{position:relative;overflow:hidden;text-align:left;border:1px solid var(--bd);border-radius:18px;padding:14px;background:var(--sf);cursor:pointer;min-height:116px;transition:.18s;animation:ptxFade .45s ease both}.pt-kpi:hover{transform:translateY(-3px);box-shadow:0 16px 40px rgba(15,23,42,.11)}.pt-kpi-glow{position:absolute;right:-30px;top:-30px;width:90px;height:90px;border-radius:999px;background:rgba(37,99,235,.14);filter:blur(3px)}.pt-kpi-icon{font-size:20px}.pt-kpi-label{display:block;margin-top:8px;color:var(--tx3);font-size:10px;text-transform:uppercase;font-weight:900;letter-spacing:.08em}.pt-kpi b{display:block;font-size:27px;letter-spacing:-.04em;margin-top:2px}.pt-kpi small{color:var(--tx2);font-size:11px}.pt-kpi.red .pt-kpi-glow{background:rgba(239,68,68,.16)}.pt-kpi.amber .pt-kpi-glow{background:rgba(245,158,11,.18)}.pt-kpi.green .pt-kpi-glow{background:rgba(34,197,94,.16)}.pt-kpi.purple .pt-kpi-glow{background:rgba(124,58,237,.16)}
      .pt-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:14px;margin-top:14px}.pt-card{background:var(--sf);border:1px solid var(--bd);border-radius:20px;padding:15px;box-shadow:0 12px 36px rgba(15,23,42,.06);animation:ptxFade .5s ease both}.pt-card h3{margin:0 0 12px;font-size:14px;letter-spacing:-.02em}.pt-card-head{display:flex;align-items:center;justify-content:space-between;gap:10px}.pt-chip{font-size:10px;font-weight:900;color:var(--tx3);background:var(--sf2);border:1px solid var(--bd);border-radius:999px;padding:4px 8px}.pt-list{display:flex;flex-direction:column;gap:9px}.pt-project-row{display:grid;grid-template-columns:1fr 80px 74px;gap:10px;align-items:center;padding:10px;border:1px solid var(--bd2);border-radius:14px;background:linear-gradient(180deg,var(--sf),var(--sf2))}.pt-bar{height:8px;border-radius:999px;background:var(--sf3);overflow:hidden}.pt-bar i{display:block;height:100%;background:linear-gradient(90deg,#2563eb,#7c3aed);border-radius:inherit}.pt-status{font-size:10px;font-weight:900;text-align:center;border-radius:999px;padding:4px 6px}.pt-status.risk{background:rgba(239,68,68,.12);color:#dc2626}.pt-status.watch{background:rgba(245,158,11,.12);color:#b45309}.pt-status.ok{background:rgba(34,197,94,.12);color:#15803d}.pt-insight{display:flex;gap:10px;padding:11px;border-radius:15px;border:1px solid var(--bd2);background:var(--sf2)}.pt-insight b{font-size:12px}.pt-insight p{margin:2px 0 0;color:var(--tx2);font-size:11px;line-height:1.35}.pt-dot{width:10px;height:10px;border-radius:99px;margin-top:3px;background:#2563eb;box-shadow:0 0 0 5px rgba(37,99,235,.10)}.pt-work{display:grid;grid-template-columns:1fr 52px;gap:9px;align-items:center}.pt-work small{color:var(--tx3);font-weight:800}.pt-work b{font-size:12px}.pt-work .pt-bar{height:9px}.pt-skeleton{border-radius:18px;background:linear-gradient(90deg,var(--sf2),var(--sf3),var(--sf2));background-size:200% 100%;animation:ptxShimmer 1.2s infinite}.pt-empty-mini{padding:18px;text-align:center;color:var(--tx3);font-size:12px}.pt-toast{position:fixed;right:22px;bottom:22px;z-index:99999;display:flex;align-items:center;gap:10px;background:#0f172a;color:white;border-radius:16px;padding:12px 14px;box-shadow:0 18px 60px rgba(15,23,42,.35);animation:ptxFade .2s ease}.pt-toast span{font-size:12px;color:rgba(255,255,255,.78)}.pt-toast button{border:0;background:rgba(255,255,255,.12);color:white;border-radius:8px;cursor:pointer}.pt-cmd-backdrop{position:fixed;inset:0;background:rgba(15,23,42,.36);backdrop-filter:blur(8px);z-index:99998;display:flex;align-items:flex-start;justify-content:center;padding-top:8vh}.pt-cmd{width:min(680px,calc(100vw - 34px));border-radius:22px;background:var(--sf);border:1px solid var(--bd);box-shadow:0 30px 100px rgba(0,0,0,.28);overflow:hidden}.pt-cmd-head{display:flex;align-items:center;gap:10px;padding:12px;border-bottom:1px solid var(--bd)}.pt-cmd-head input{flex:1;border:0;outline:0;background:transparent;color:var(--tx);font-size:15px}.pt-cmd-head button{border:1px solid var(--bd);background:var(--sf2);border-radius:9px;color:var(--tx2)}.pt-cmd-list{padding:8px;max-height:380px;overflow:auto}.pt-cmd-list button{width:100%;display:flex;justify-content:space-between;gap:14px;padding:11px;border:0;border-radius:13px;background:transparent;color:var(--tx);cursor:pointer;text-align:left}.pt-cmd-list button:hover{background:var(--sf2)}.pt-cmd-list small{color:var(--tx3)}
      @media(max-width:1100px){.pt-hero-grid,.pt-grid{grid-template-columns:1fr}.pt-kpis{grid-template-columns:repeat(2,1fr)}}@media(max-width:640px){.pt-kpis{grid-template-columns:1fr}.ptx-shell{padding:12px}.pt-hero h1{font-size:23px}}
    `}</style>
    <${PTToast} msg=${toast} onClose=${()=>setToast(null)}/><${PTCommandPalette} open=${cmd} onClose=${()=>setCmd(false)} onNav=${onNav} items=${cmdItems}/>
    <section class="pt-hero"><div class="pt-hero-grid"><div><span class="pt-eyebrow">⚡ Enterprise command center</span><h1>Good day, ${(cu&&cu.name||'there').split(' ')[0]} — your portfolio is ${health>=80?'healthy':health>=60?'watching a few risks':'asking for attention'}.</h1><p>${activeTeam?activeTeam.name+' team view · ':''}${k.projects||p.length} projects, ${k.active_tasks||0} active tasks, ${k.open_tickets||0} open tickets. Use the command palette for fast navigation.</p><div class="pt-actions"><button class="pt-action" onClick=${()=>setCmd(true)}>⌘K Command</button><button class="pt-action" onClick=${()=>onNav&&onNav('timeline')}>📅 Timeline</button><button class="pt-action" onClick=${()=>onNav&&onNav('tasks:stage:blocked')}>🚧 Blockers</button><button class="pt-action" onClick=${()=>setToast({title:'AI Summary',body:insights[0]?insights[0].detail:'Workspace looks stable.'})}>✨ AI Summary</button>${safe(teams).length?html`<div ref=${teamDropRef} style=${{position:'relative'}}><button class="pt-action" onClick=${()=>setTeamDropOpen(v=>!v)}>${activeTeam?'👥 '+activeTeam.name:'👥 All teams'}</button>${teamDropOpen?html`<div style=${{position:'absolute',right:0,top:'calc(100% + 8px)',width:260,zIndex:20,background:'var(--sf)',border:'1px solid var(--bd)',borderRadius:16,padding:8,boxShadow:'0 20px 60px rgba(15,23,42,.18)'}}><input class="inp" style=${{height:30,fontSize:11,marginBottom:6}} placeholder="Search teams" value=${teamSearch} onInput=${e=>setTeamSearch(e.target.value)}/><button class="pt-action" style=${{width:'100%',textAlign:'left'}} onClick=${()=>{setTeamCtx&&setTeamCtx(null);setTeamDropOpen(false)}}>All teams</button>${filteredTeams.map(tm=>html`<button class="pt-action" style=${{width:'100%',textAlign:'left',marginTop:5}} onClick=${()=>{setTeamCtx&&setTeamCtx(tm.id);setTeamDropOpen(false)}}>${tm.name}</button>`)}</div>`:null}</div>`:null}</div></div><div class="pt-health"><div style=${{display:'flex',justifyContent:'space-between',alignItems:'start'}}><div><small>Portfolio health</small><div class="pt-score">${health}</div></div><span class="pt-chip" style=${{background:'rgba(255,255,255,.1)',color:'white',border:0}}>Live</span></div><div class="pt-ring" style=${{'--w':health+'%'}}><i></i></div><div class="pt-risk"><span>Risk score ${risk}</span><span>${cc?fmtD(cc.generated_at):'Loading'}</span></div></div></div></section>
    ${loading?html`<div class="pt-kpis"><${PTSkeleton}/><${PTSkeleton}/><${PTSkeleton}/><${PTSkeleton}/><${PTSkeleton}/></div><div class="pt-grid"><${PTSkeleton} h=${330}/><${PTSkeleton} h=${330}/></div>`:html`
      <div class="pt-kpis"><${PTKpi} label="Projects" value=${k.projects||0} sub="active portfolio" icon="📁" tone="blue" onClick=${()=>onNav&&onNav('projects')}/><${PTKpi} label="Active Tasks" value=${k.active_tasks||0} sub=${mkSub(k.completed_tasks,'completed')} icon="⚡" tone="purple" onClick=${()=>onNav&&onNav('tasks')}/><${PTKpi} label="Blocked" value=${k.blocked_tasks||0} sub="needs unblock" icon="🚧" tone="red" onClick=${()=>onNav&&onNav('tasks:stage:blocked')}/><${PTKpi} label="Due Soon" value=${k.due_soon_tasks||0} sub=${mkSub(k.overdue_tasks,'overdue')} icon="⏳" tone="amber" onClick=${()=>onNav&&onNav('timeline')}/><${PTKpi} label="Tickets" value=${k.open_tickets||0} sub="open / unresolved" icon="🎫" tone="green" onClick=${()=>onNav&&onNav('tickets')}/></div>
      <div class="pt-grid"><div class="pt-card"><div class="pt-card-head"><h3>Portfolio delivery map</h3><span class="pt-chip">Progress vs risk</span></div><div class="pt-list">${topProjects.length?topProjects.map(pr=>html`<div class="pt-project-row" onClick=${()=>onNav&&onNav('projects',pr.id)}><div><b style=${{fontSize:12}}>${pr.name}</b><div class="pt-bar" style=${{marginTop:7}}><i style=${{width:(pr.progress||0)+'%'}}></i></div><small style=${{color:'var(--tx3)'}}>Tasks ${pr.done}/${pr.tasks} · blocked ${pr.blocked}</small></div><b>${pr.progress||0}%</b><span class=${'pt-status '+(pr.status==='At Risk'?'risk':pr.status==='Watch'?'watch':'ok')}>${pr.status}</span></div>`):html`<div class="pt-empty-mini">Create projects to see delivery health here.</div>`}</div></div>
      <div class="pt-card"><div class="pt-card-head"><h3>AI investigation notes</h3><span class="pt-chip">Generated</span></div><div class="pt-list">${insights.map((x,i)=>html`<div class="pt-insight"><span class="pt-dot" style=${{background:x.type==='risk'||x.type==='blocker'?'#dc2626':x.type==='deadline'?'#d97706':'#2563eb'}}></span><div><b>${x.title}</b><p>${x.detail}</p></div></div>`)}</div></div></div>
      <div class="pt-grid"><div class="pt-card"><div class="pt-card-head"><h3>Throughput and capacity</h3><span class="pt-chip">14 days</span></div>${RC&&RC.ResponsiveContainer?html`<${RC.ResponsiveContainer} width="100%" height=${240}><${RC.LineChart} data=${(cc&&cc.throughput&&cc.throughput.length)?cc.throughput:[{day:'Mon',created:2,completed:1,hours:4},{day:'Tue',created:3,completed:2,hours:6},{day:'Wed',created:1,completed:3,hours:5},{day:'Thu',created:4,completed:2,hours:7},{day:'Fri',created:2,completed:4,hours:3}]} margin=${{top:10,right:10,left:-20,bottom:0}}><${RC.CartesianGrid} strokeDasharray="3 3" opacity=${0.25}/><${RC.XAxis} dataKey="day" fontSize=${10}/><${RC.YAxis} fontSize=${10}/><${RC.Tooltip}/><${RC.Line} type="monotone" dataKey="created" stroke="#2563eb" strokeWidth=${2} dot=${false}/><${RC.Line} type="monotone" dataKey="completed" stroke="#16a34a" strokeWidth=${2} dot=${false}/><${RC.Line} type="monotone" dataKey="hours" stroke="#7c3aed" strokeWidth=${2} dot=${false}/></${RC.LineChart}></${RC.ResponsiveContainer}>`:html`<div class="pt-empty-mini">Charts unavailable.</div>`}</div>
      <div class="pt-card"><div class="pt-card-head"><h3>Workload matrix</h3><span class="pt-chip">Top 12</span></div><div class="pt-list">${workload.length?workload.map(w=>html`<div class="pt-work"><div><div style=${{display:'flex',justifyContent:'space-between'}}><b>${w.name}</b><small>${w.role||''}</small></div><div class="pt-bar" style=${{marginTop:6}}><i style=${{width:Math.min(100,(w.active||0)*10)+'%',background:w.color}}></i></div><small>${w.active} active · ${w.blocked} blocked · ${w.critical} critical</small></div><b>${w.active}</b></div>`):html`<div class="pt-empty-mini">No team workload available.</div>`}</div></div></div>
      <div class="pt-grid"><div class="pt-card"><div class="pt-card-head"><h3>Stage distribution</h3><span class="pt-chip">Kanban</span></div>${RC&&RC.ResponsiveContainer?html`<${RC.ResponsiveContainer} width="100%" height=${230}><${RC.BarChart} data=${stageData}><${RC.XAxis} dataKey="name" fontSize=${10}/><${RC.YAxis} fontSize=${10}/><${RC.Tooltip}/><${RC.Bar} dataKey="value" fill="#2563eb" radius=${[8,8,0,0]}/></${RC.BarChart}></${RC.ResponsiveContainer}>`:null}</div><div class="pt-card"><div class="pt-card-head"><h3>Priority pressure</h3><span class="pt-chip">Risk radar</span></div>${RC&&RC.ResponsiveContainer?html`<${RC.ResponsiveContainer} width="100%" height=${230}><${RC.RadarChart} data=${priData}><${RC.PolarGrid}/><${RC.PolarAngleAxis} dataKey="name" fontSize=${10}/><${RC.PolarRadiusAxis} fontSize=${10}/><${RC.Radar} dataKey="value" stroke="#7c3aed" fill="#7c3aed" fillOpacity=${0.25}/><${RC.Tooltip}/></${RC.RadarChart}></${RC.ResponsiveContainer}>`:null}</div></div>`}
  </div>`;
}
